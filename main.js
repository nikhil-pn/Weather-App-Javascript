document.addEventListener("DOMContentLoaded", async () => {
  ...
  const getHourlyForecast = async () => {
    let response = await fetch(
      `https://api.openweathermap.org/data/2.5/forecast?q=london&appid=0e1cc6e24661f917b43a7d4c41bbe50b&units=metric&`
  ...
  const getWeather = async () => {
    let response = await fetch(
      `https://api.openweathermap.org/data/2.5/weather?q=london&appid=0e1cc6e24661f917b43a7d4c41bbe50b&units=metric&`
    ).then((response) => response.json());
